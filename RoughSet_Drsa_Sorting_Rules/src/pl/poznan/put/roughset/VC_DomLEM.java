...
public class VC_DomLEM {
    ...
    List<Rule> runAlgorithm() {
        for (Union union : unions) {
            ...
            removeNonMinimalRules(allUnionsRules);
        }
        return allUnionsRules;
    }

    private void removeNonMinimalRules(List<Rule> allUnionsRules) {
        Iterator<Rule> i = allUnionsRules.iterator();
        while (i.hasNext()) {
            Rule potentiallyUnnecessaryRule = i.next();
            List<Rule> otherRules = new ArrayList<>(allUnionsRules);
            otherRules.remove(potentiallyUnnecessaryRule);
            for (Rule rule : otherRules) {
                if (rule.hasNotLessSpecificDecisionThan(potentiallyUnnecessaryRule) && rule.hasNotLessGeneralConditionsThan(potentiallyUnnecessaryRule)) {
                    i.remove();
                }
            }
        }
    ...
    private List<Rule> VC_SequentialCovering(Union union, List<Alternative> allowedObjects, double consistencyThreshold) {
        ...
        while (notCoveredYet.size() > 0) {
            Rule currentRule = new Rule(union.unionType, union.concept, union.conceptRank, ruleType);
